...
- Dormant scaffold added after this scoping pass: `gadget_internal_continue` consumes `[guest_pc][internal_code_ptr]`, sets `CPU_pc`, sets `_pc` to the internal continuation pointer, and `gret`s without using `fiber_ret_chain`. `gen_state` now has zero-initialized internal-continue patch arrays so future generation can store offsets while `state->block` is reallocatable and let `gen_end` patch final code-stream pointers. No generator call site emits this gadget yet, so true-superblock generation remains disabled.
